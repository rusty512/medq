...
function saveJsonData(filename, data) {
  ...
  console.log(`✅ ${filename} sauvegardé (${data.length || Object.keys(data).length} éléments)`);
}
      ...
      if (code.liste_attri?.liste_elm_mesur?.elm_mesur) {
        ...
        for (const elm of elmList) {
          codeData.liste_elm_mesur.push({
            ...
            df_effec_elm_mesur: elm.df_effec_elm_mesur || null
          });
        }
      }
      ...
      if (code.liste_attri?.liste_role_spec?.role_spec) {
        ...
        for (const role of roleList) {
          codeData.liste_role_spec.push({
            ...
            df_effec_role: role.df_effec_role || null
          });
        }
      }
...
function extractContextElements() {
  ...
  const elements = [];
  const rootData = jsonData.elm_contx;
  ...
  if (rootData && rootData.val_elm_contx) {
    ...
    for (const element of elementList) {
      const elementData = {
        ...
        txt_elm_contx: element.txt_elm_contx,
        cod_niv: element.cod_niv,
        dd_effec_elm_contx: element.dd_effec_elm_contx,
        liste_util_elm_contx: []
      };
      
      // Extraire les utilisations de l'élément de contexte
      if (element.liste_util_elm_contx && element.liste_util_elm_contx.util_elm_contx) {
        const utilList = Array.isArray(element.liste_util_elm_contx.util_elm_contx)
          ? element.liste_util_elm_contx.util_elm_contx
          : [element.liste_util_elm_contx.util_elm_contx];
        
        for (const util of utilList) {
          const utilData = {
            typ_fact: util.typ_fact,
            dd_util_elm_contx: util.dd_util_elm_contx,
            liste_cod_fact: []
          };
          
          // Extraire les codes de facturation liés
          if (util.liste_cod_fact && util.liste_cod_fact.val_cod_fact) {
            const factList = Array.isArray(util.liste_cod_fact.val_cod_fact)
              ? util.liste_cod_fact.val_cod_fact
              : [util.liste_cod_fact.val_cod_fact];
            
            for (const fact of factList) {
              utilData.liste_cod_fact.push({
                cod_fact: fact.cod_fact,
                dd_effec_cod_fact: fact.dd_effec_cod_fact || null,
                df_effec_cod_fact: fact.df_effec_cod_fact || null
              });
            }
          }
          
          elementData.liste_util_elm_contx.push(utilData);
        }
      }
...
function extractExplanatoryMessages() {
  ...
  const messages = [];
  const rootData = jsonData.msg_expl;
  
  if (rootData && rootData.val_msg_expl) {
    const messageList = Array.isArray(rootData.val_msg_expl)
      ? rootData.val_msg_expl
      : [rootData.val_msg_expl];
    
    for (const message of messageList) {
      const messageData = {
        cod_msg_expl: message.cod_msg_expl,
        txt_msg_expl: message.txt_msg_expl,
        catg_msg_expl: message.catg_msg_expl,
        typ_fact: message.typ_fact || null,
        dd_txt_msg_expl: message.dd_txt_msg_expl
      };
...
function extractDiagnosticCodes() {
  ...
  if (rootData && rootData.liste_cod_diag && rootData.liste_cod_diag.cod_diag) {
    const codeList = Array.isArray(rootData.liste_cod_diag.cod_diag)
      ? rootData.liste_cod_diag.cod_diag
      : [rootData.liste_cod_diag.cod_diag];
    ...
    for (const code of codeList) {
      const codeData = {
        ...
        typ_cod_diag: code.typ_cod_diag, // CIM-9, CIM-10, etc.
...
function extractLocationCodes() {
  ...
  if (rootData && rootData.liste_localite && rootData.liste_localite.localite) {
    const locationList = Array.isArray(rootData.liste_localite.localite)
      ? rootData.liste_localite.localite
      : [rootData.liste_localite.localite];
